            ...
            <div className="bg-white p-8 shadow rounded-lg w-full max-w-md aspect-square flex flex-col justify-center">
              ...
              <div className="grid grid-cols-3 gap-4">
                {services.slice(0, 6).map((service, index) => (
                  <div
                    key={index}
                    className="flex flex-col items-center text-center bg-gray-100 p-2 rounded-lg shadow"
                  >
                    {/* Adding a link */}
                    <a
                    ...
                    >
                      <img
                        src={service.img}
                        alt={service.name}
                        className="w-14 h-14 rounded-lg object-cover mb-2 transform transition duration-300 hover:scale-105"
                      />
                    </a>
                ...
                <div className="col-span-3 flex gap-4">
                  {services.slice(6, 8).map((service, index) => (
                    <div
                      key={index}
                      className="flex flex-col items-center text-center bg-gray-100 p-2 rounded-lg shadow w-1/2"
                    >
                      <a
                        href={`/services/${service.name
                          .replace(/\s+/g, "-")
                      ...
                      >
                        <img
                          src={service.img}
                          alt={service.name}
                          className="w-16 h-16 rounded-lg object-cover mb-2 transform transition duration-300 hover:scale-105"
                        />
                      </a>
                      <span className="text-xs text-gray-700 font-medium">
                        {service.name}
          ...
          <div className="grid grid-cols-2 gap-4 md:masonry-2-cols items-start">
            ...
            <div className="flex flex-col gap-4">
              ...
              <img
                ...
                alt="Service 4"
                className="rounded-lg w-full object-cover h-[360px]"
              />
            </div>
          </div>
        </div>
